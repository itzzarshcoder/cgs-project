...
export default function OrderManagement() {
  ...
  const handleDateRangeSearch = async () => {
    ...
    try {
      // @ts-ignore
      const response = await getOrdersByDateRange(dateRange.start, dateRange.end);
      console.log("📅 Date Range Response:", response);

      let orderData = [];
      
      // Robust parsing similar to fetchOrders
      ...
      setTotalPages(1); // Disable pagination for date range view
      setDateFilterOpen(false);
      toast.success(`Found ${orderData.length} orders`);
    } catch (error: any) {
